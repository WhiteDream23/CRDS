    ...
    def _generate_data(self):
        ...
        for _ in tqdm(range(self.num_samples), desc="Generating Augmented Data"):
            ...
            if self.augmentations.get('realistic_shot_noise', False):
                ...
                noise_level = base_gamma*2
                total_noise=generate_poisson_like_noise(ideal_decay_curve, noise_level)
...
def generate_poisson_like_noise(clean_signal, noise_level):
    """
    生成一个“类泊松”噪声（信号依赖的高斯噪声）。
    其核心特性是：噪声的方差与信号强度成正比。
    ...
    Returns:
        np.array: 生成的仿真噪声。
    """
    # 1. 生成基础的标准高斯随机数 (均值为0, 标准差为1)
    n_points = len(clean_signal)
    base_gaussian_noise = np.random.normal(0, 1, size=n_points)

    # 2. 施加信号依赖性（噪声标准差与信号强度的平方根成正比）
    signal_dependency = np.sqrt(np.maximum(clean_signal, 0))

    # 3. 结合总体强度系数，生成最终噪声
    final_noise = base_gaussian_noise * signal_dependency * noise_level
...
if __name__ == '__main__':
    # --- 参数设置 ---
    output_dir = Path("dataset_visualization6")
