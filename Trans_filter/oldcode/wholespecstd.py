...
def analyze_tau_columns(csv_file_path):
    ...
    target_columns = [col for col in df.columns if 'Tau_mean' in col]

    if not target_columns:
        print("错误: 在文件中未找到任何列名包含 'Tau_mean' 的列。")
        return

    print(f"找到目标列: {target_columns}\n")

    # 3. 选择指定的行范围
    rows_part1 = df.iloc[60:90]  # 选取第 0 到 40 行
    rows_part2 = df.iloc[24:44]  # 选取第 60 到 90 行
...
if __name__ == "__main__":
    ...
    csv_path = r"E:\pythonProject_crds\Trans_filter\processed\modelper_file_summary-0709.csv"
