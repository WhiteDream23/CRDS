...
class ChunkedRingdownDataset(Dataset):
    ...
    def __init__(self, chunks_dir="temp_chunks", transform=None, shuffle_chunks=True, normalize_targets=True,
                 tau_min=5.0, tau_max=25.0, cache_size=10):
        """
        初始化数据集
...
class InMemoryRingdownDataset(Dataset):
    ...
    def __init__(self, chunks_dir="temp_chunks", transform=None, shuffle=True, normalize_targets=True, tau_min=5.0,
                 tau_max=25.0):
        """初始化数据集并将所有数据加载到内存中"""
        self.transform = transform
        self.normalize_targets = normalize_targets
...
def create_dataloaders(chunks_dir="temp_chunks6", batch_size=32, train_ratio=0.8, use_in_memory=True, mintau=5.0,
                       maxtau=25.0):
    """创建训练和验证数据加载器"""
    if use_in_memory:
        numworkers = 0
        dataset = InMemoryRingdownDataset(chunks_dir=chunks_dir, transform=SignalTransform(), tau_max=maxtau,
                                          tau_min=mintau)
    else:
        numworkers = 0
        dataset = ChunkedRingdownDataset(chunks_dir=chunks_dir, transform=SignalTransform(), tau_max=maxtau,
                                         tau_min=mintau, cache_size=100)
...
def test_cnn_model():
    ...
    train_loader, val_loader = create_dataloaders(
        chunks_dir="temp_chunks_10-20_1200_0416_0.005",
        batch_size=128,
        train_ratio=0.8,
        mintau=10,
        maxtau=20,
    ...
    best_model_path = 'phy_best_ringdown_model2_10-20-0.005.pth'
    train_losses = []
    val_losses = []

    # 添加物理损失权重 (超参数)
    lambda_physics = 0.05  # 需要调整

    # 获取或生成时间轴张量 (假设与输入信号长度一致)
    # 假设 num_points 是已知的信号长度，例如 800
    num_points = 1200  # 或者从     train_loader.dataset 获取
    t_numpy = np.linspace(0, 120, num_points, dtype=np.float32)  # 与 data_generation.py 一致
    t_tensor = torch.from_numpy(t_numpy).to(device)  # 移到设备

    # 获取tau的归一化范围 (如果使用了归一化)
    tau_min, tau_max = 10.0, 20.0  # 需要与 create_dataloaders 中一致

    # --- 训练循环 ---
    for epoch in range(num_epochs):
        model.train()
        running_loss = 0.0
        running_data_loss = 0.0  # 用于记录
        running_physics_loss = 0.0  # 用于记录
        batchnum = 0
        for inputs, targets in train_loader:
            batchnum += 1
            inputs, targets = inputs.to(device, torch.float32), targets.to(device, torch.float32)
            # inputs 形状: (batch_size, seq_len)，假设 SignalTransform 已应用
            # targets 形状: (batch_size,)，假设是归一化的 tau [0,1]
            targets = targets.squeeze()
            optimizer.zero_grad()

            # 1. 模型预测 (输出是归一化的 tau)
            outputs_normalized = model(inputs)  # 形状: (batch_size,)

            # 2. 计算数据损失 (Data Loss)
            # 确保 targets 也是 float32
            data_loss = criterion(outputs_normalized, targets)

            # 3. 计算物理损失 (Physics Loss)
            physics_loss = torch.tensor(0.0, device=device)  # 初始化
            if lambda_physics > 0:
                with torch.no_grad():  # 物理损失的计算不应影响模型预测的梯度，只用来约束
                    # a. 反归一化预测的 tau
                    tau_pred_unnormalized = outputs_normalized * (tau_max - tau_min) + tau_min
                    # 添加小的epsilon防止tau为0或太小导致exp溢出或除零
                    tau_pred_unnormalized = torch.clamp(tau_pred_unnormalized, min=1e-6)

                    # b. 估计 I0_est
                    # 因为输入 inputs 已经归一化到 [0,1]，可以假设 I0_est = 1.0
                    I0_est = torch.tensor(1.0, device=device)

                    # c. 准备时间轴和tau的形状用于广播
                    # t_tensor 形状: (seq_len,) -> (1, seq_len)
                    # tau_pred_unnormalized 形状: (batch_size,) -> (batch_size, 1)
                    time_expanded = t_tensor.unsqueeze(0)
                    tau_expanded = tau_pred_unnormalized.unsqueeze(1)
                    I0_expanded = I0_est  # 标量可以广播

                # d. 重构理想信号 (归一化尺度)
                # I_reconstructed 形状: (batch_size, seq_len)
                I_reconstructed = I0_expanded * torch.exp(-time_expanded / tau_expanded)

                # 在计算 physics_loss = criterion(I_reconstructed, inputs) 之前加入对齐
                with torch.no_grad():
                    # 防止除零，并确保在同一设备
                    input_start = torch.clamp(inputs[:, 0:1], min=1e-6)
                    recon_start = torch.clamp(I_reconstructed[:, 0:1], min=1e-6)
                    # 计算缩放因子，确保与 inputs[:, 0:1] 维度匹配以进行广播
                    scale_factor = input_start / recon_start  # 形状 (batch_size, 1)
                # 应用缩放因子进行广播
                I_reconstructed_aligned = I_reconstructed * scale_factor

                # e. 计算物理损失 (重构信号 vs 输入信号)
                # inputs 已经是归一化的，形状 (batch_size, seq_len)
                # 使用与 data_loss 相同的损失函数 criterion
                physics_loss = physics_criterion(I_reconstructed_aligned, inputs)
            ...
            if batchnum % 100 == 0:
                print(f"Batch {batchnum}/{len(train_loader)}, "
        ...
        train_losses.append(avg_train_loss)  # 可以考虑也记录 data 和 physics loss

        # --- 验证循环 (通常只评估 Data Loss) ---
        model.eval()
        val_loss = 0.0  # 这里仍然基于原始的 data loss 进行评估和模型选择
        with torch.no_grad():
            for inputs, targets in val_loader:
                inputs, targets = inputs.to(device, torch.float32), targets.to(device, torch.float32)
                targets = targets.squeeze()
                outputs_normalized = model(inputs)
                loss = criterion(outputs_normalized, targets)
                val_loss += loss.item()
        if len(val_loader) > 0:
        ...
        print(f'Epoch {epoch + 1}/{num_epochs}, '
              f'Avg Train Total Loss: {avg_train_loss:.6f}, '
              f'Avg Train Data Loss: {avg_data_loss:.6f}, '
              f'Avg Train Physics Loss: {avg_physics_loss:.6f}, '
              f'Val (Data) Loss: {val_loss:.6f}')  # 明确验证损失是 Data Loss
        # 验证阶段结束后，检查是否为最佳模型
        ...
        if epoch % 5 == 0 or epoch == num_epochs - 1:
            ...
            plt.xlabel('真实值')
            plt.ylabel('预测值')
            plt.savefig(f'phy2_10-20-0.005_pred_vs_true_epoch_{epoch + 1}.png')
            plt.close()
    ...
    plt.savefig('phy2_10-20-CNN_model_training_results_0.005.png', dpi=300)
    plt.close()
    # 训练结束后加载最佳模型
    ...
    plt.title(
        f'最佳模型预测结果 (Epoch {checkpoint["epoch"]})\nMSE: {final_mse:.6f}, MAE: {final_mae:.6f}, R²: {final_r2:.6f}')
    plt.xlabel('真实值')
    plt.ylabel('预测值')
    plt.grid(True, alpha=0.3)
    plt.savefig('phy2_10-20-best_model_predictions_0.005.png', dpi=300)
    plt.close()
